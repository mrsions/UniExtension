...
using EditorGUICompat = UniExtension.Editor.Internal.UniEditorGUI;
using EditorGUIUtilityCompat = UniExtension.Editor.Internal.UniEditorGUIUtility;
using EditorGUILayoutCompat = UniExtension.Editor.Internal.UniEditorGUILayout;
using EditorStylesCompat = UniExtension.Editor.Internal.UniEditorStyles;
using GUIContentCompat = UniExtension.Editor.Internal.UniGUIContent;
using GUIClipCompat = UniExtension.Editor.Internal.UniGUIClip;
using EventCommandNamesCompat = UniExtension.Editor.Internal.UniEventCommandNames;
using LogEntries = UniExtension.Editor.Internal.LogEntries;
using LogEntry = UniExtension.Editor.Internal.LogEntry;
using LogEntryStruct = UniExtension.Editor.Internal.LogEntryStruct;
using ConsoleWindowUtilityCompat = UniExtension.Editor.Internal.ConsoleWindowUtility;
using UnityEngine;
...
{
    [EditorWindowTitle(title = "UniConsole", useTypeNameAsIconName = false)]
    internal class UniConsoleWindow : EditorWindow, IHasCustomMenu
    {
        ...
        static UniConsoleWindow ms_ConsoleWindow = null;
        private string m_SearchText;

        static readonly int k_HasSpaceForExtraButtonsCutoff = 420;
        static readonly float k_SplitterHeight = 3f;

        public static void ShowConsoleWindow(bool immediate)
        {
            if (!ms_ConsoleWindow)
            {
                ms_ConsoleWindow = ScriptableObject.CreateInstance<UniConsoleWindow>();
        ...
        static internal void LoadIcons()
        {
            ...
            ms_LoadedIcons = true;
            iconInfo = EditorGUIUtilityCompat.LoadIcon("console.infoicon");
            iconWarn = EditorGUIUtilityCompat.LoadIcon("console.warnicon");
            iconError = EditorGUIUtilityCompat.LoadIcon("console.erroricon");
            iconInfoSmall = EditorGUIUtilityCompat.LoadIcon("console.infoicon.sml");
            iconWarnSmall = EditorGUIUtilityCompat.LoadIcon("console.warnicon.sml");
            iconErrorSmall = EditorGUIUtilityCompat.LoadIcon("console.erroricon.sml");
            ...
            iconInfoMono = EditorGUIUtilityCompat.LoadIcon("console.infoicon.inactive.sml");
            iconWarnMono = EditorGUIUtilityCompat.LoadIcon("console.warnicon.inactive.sml");
            iconErrorMono = EditorGUIUtilityCompat.LoadIcon("console.erroricon.inactive.sml");
            Constants.Init();
        }
        ...
        public static void LogChanged()
        {
            ...
            ConsoleWindowUtilityCompat.Internal_CallLogsHaveChanged();
        }

        public UniConsoleWindow()
        {
        ...
        private static GUIContent GetLocalizedTitleContent()
        {
            var icon = EditorGUIUtility.IconContent("console.infoicon.sml")?.image
                ?? EditorGUIUtility.IconContent("console.infoicon")?.image;
            return new GUIContent("UniConsole", icon);
        }
        ...
        void SetActiveEntry(LogEntry entry)
        {
            if (entry != null)
            {
                ...
                if (entry.entityId != UnityEngine.EntityId.None && m_LastPingedEntryRow != entryRow)
                {
        ...
        private void GetVerticalSplitRects(Rect totalRect, Event e, out Rect topRect, out Rect splitterRect, out Rect bottomRect)
        {
            float availableHeight = Mathf.Max(0f, totalRect.height - k_SplitterHeight);
            float topMin = spl.minSizes.Length > 0 ? spl.minSizes[0] : 0f;
            float bottomMin = spl.minSizes.Length > 1 ? spl.minSizes[1] : 0f;

            float topSize;
            float relativeTop = spl.relativeSizes.Length > 0 ? spl.relativeSizes[0] : 1f;
            float relativeBottom = spl.relativeSizes.Length > 1 ? spl.relativeSizes[1] : 1f;
            float totalRelative = Mathf.Max(0.0001f, relativeTop + relativeBottom);
            topSize = availableHeight * (relativeTop / totalRelative);
            topSize = Mathf.Clamp(topSize, topMin, Mathf.Max(topMin, availableHeight - bottomMin));

            splitterRect = new Rect(totalRect.x, totalRect.y + topSize, totalRect.width, k_SplitterHeight);
            ...
            switch (e.GetTypeForControl(controlId))
            {
                ...
                case EventType.MouseDrag:
                    ...
                    {
                        topSize = Mathf.Clamp(e.mousePosition.y - totalRect.y, topMin, Mathf.Max(topMin, availableHeight - bottomMin));
                        spl.relativeSizes[0] = topSize;
                        spl.relativeSizes[1] = availableHeight - topSize;
                        splitterRect.y = totalRect.y + topSize;
                        e.Use();
                    }
                    break;
                ...
                case EventType.Repaint:
                    EditorGUIUtility.AddCursorRect(splitterRect, MouseCursor.ResizeVertical, controlId);
                    EditorGUI.DrawRect(splitterRect, EditorGUIUtility.isProSkin ? new Color(0.22f, 0.22f, 0.22f) : new Color(0.7f, 0.7f, 0.7f));
                    break;
            }

            topRect = new Rect(totalRect.x, totalRect.y, totalRect.width, topSize);
            bottomRect = new Rect(totalRect.x, splitterRect.yMax, totalRect.width, Mathf.Max(0f, totalRect.yMax - splitterRect.yMax));
        }
        ...
        internal void OnGUI()
        {
            ...
            if (EditorGUILayoutCompat.DropDownToggle(ref clearClicked, Constants.Clear, EditorStylesCompat.ToolbarDropDownToggle))
            {
            ...
            {
                Rect r = new Rect(0, EditorGUIUtilityCompat.SingleLineHeight, ms_ConsoleWindow.position.width, ms_ConsoleWindow.position.height - EditorGUIUtilityCompat.SingleLineHeight);
                GUI.Box(r, m_ConsoleSearchNoResultMsg, Constants.ConsoleSearchNoResult);
            }
            else
            {
                Rect splitRect = GUILayoutUtility.GetRect(0, 100000, 0, 100000, GUILayout.ExpandWidth(true), GUILayout.ExpandHeight(true));
                GetVerticalSplitRects(splitRect, e, out Rect listRect, out _, out Rect detailRect);
                ...
                using (new GettingLogEntriesScope(m_ListView))
                {
                    ...
                    foreach (ListViewElement el in ListViewGUI.ListView(m_ListView,
                        ListViewOptions.wantsRowMultiSelection, Constants.Box, GUILayout.ExpandWidth(true), GUILayout.ExpandHeight(true)))
                    {
                        ...
                        else if (e.type == EventType.Repaint)
                        {
                            ...
                            else if (text != null)
                            {
                                ...
                                else // the searchtext is visible, we show the selection
                                {
                                    ...
                                    EditorGUICompat.DrawWithTextSelection(errorModeStyle, textRect, tempContent, isActive,
                    ...
                    else
                    {
                        ...
                        if (e.type == EventType.ExecuteCommand && e.commandName == EventCommandNamesCompat.Copy &&
                ...
                m_TextScroll = GUILayout.BeginScrollView(m_TextScroll, Constants.Box, GUILayout.ExpandWidth(true), GUILayout.ExpandHeight(true));
                ...
                float contentWidth = Mathf.Max(1f, GUIClipCompat.VisibleRect.width - 16f);
                float contentHeight = Constants.MessageStyle.CalcHeight(GUIContentCompat.Temp(stackWithHyperlinks), contentWidth);
                Rect contentRect = GUILayoutUtility.GetRect(contentWidth, contentHeight, GUILayout.ExpandWidth(true));
                EditorGUI.SelectableLabel(contentRect, stackWithHyperlinks, Constants.MessageStyle);
            ...
            if ((e.type == EventType.ValidateCommand || e.type == EventType.ExecuteCommand) && e.commandName == EventCommandNamesCompat.Copy && m_CopyString != null)
            {
        ...
        private void SearchField(Event e)
        {
            ...
            if (e.commandName == EventCommandNamesCompat.Find)
            {
                if (e.type == EventType.ExecuteCommand)
                {
            ...
            Rect rect = GUILayoutUtility.GetRect(0, EditorGUILayoutCompat.LabelFloatMaxW * 1.5f, EditorGUIUtilityCompat.SingleLineHeight,
                EditorGUIUtilityCompat.SingleLineHeight, EditorStyles.toolbarSearchField, GUILayout.MinWidth(60),
                GUILayout.MaxWidth(300));
            var filteringText = EditorGUICompat.ToolbarSearchField(rect, searchText, false);
                    ...
                    if (lines[i][filePathIndex] != '<') // sometimes no url is given, just an id between <>, we can't do an hyperlink
                    {
                        ...
                        if (lineIndex > 0)
                        {
                            ...
                            if (endLineIndex > 0)
                            {
                                ...
                                textWithHyperlinks.Append($"{lines[i].Substring(0, filePathIndex)}<color={EditorGUIUtilityCompat.GetHyperlinkColorForSkin()}><link=\"href='{filePath}' line='{lineString}'\">{filePath}:{lineString}</link></color>)\n");
