...
namespace UnityEditor
{
    ...
    internal class ListViewGUI
    {
        ...
        static public ListViewShared.ListViewElementsEnumerator DoListView(Rect pos, ListViewState state, int[] colWidths, string dragTitle)
        {
            ...
            Rect visibleRect = GUIClipCompat.VisibleRect;
            if ((visibleRect.x < 0) || (visibleRect.y < 0)) // TODO: this is needed for simple LVs to work. we are not in a clip at all.
            {
                vRect = pos;
            }
            else
                vRect = (pos.y < 0) ? new Rect(0, 0, visibleRect.width, visibleRect.height) : new Rect(0, state.scrollPos.y, visibleRect.width, visibleRect.height); // check if this is custom scroll
