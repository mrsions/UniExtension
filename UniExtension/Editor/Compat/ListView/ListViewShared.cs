...
using UnityEngine;
using DragAndDropDelayCompat = UniExtension.Editor.Internal.UniDragAndDropDelay;
using GUIClipCompat = UniExtension.Editor.Internal.UniGUIClip;
using GUILayoutUtilityCompat = UniExtension.Editor.Internal.UniGUILayoutUtility;
            ...
            public bool MoveNext()
            {
                if (xPos > -1)
                {
                    if (ListViewShared.HasMouseDown(ilvState, rect))
                    {
                        ...
                        {
                            DragAndDropDelayCompat delay = (DragAndDropDelayCompat)GUIUtility.GetStateObject(typeof(DragAndDropDelayCompat), ilvState.state.ID);
                    ...
                    if (!ListViewShared.isDragging &&
                        ...
                        GUIClipCompat.VisibleRect.Contains(Event.current.mousePosition))
                    {
                        DragAndDropDelayCompat delay = (DragAndDropDelayCompat)GUIUtility.GetStateObject(typeof(DragAndDropDelayCompat), ilvState.state.ID);
                ...
                if (quiting)
                {
                    ...
                    if (ilvState.wantsReordering && (GUIUtility.hotControl == ilvState.state.ID))
                    {
                        ...
                        switch (Event.current.type)
                        {
                            ...
                            {
                                if (GUIClipCompat.VisibleRect.Contains(Event.current.mousePosition))
                                {
                    ...
                    else if (ilvState.wantsToAcceptCustomDrag && (dragControlID != ilvState.state.ID))
                    {
                        switch (Event.current.type)
                        {
                            case EventType.DragUpdated:
                            {
                                ...
                                if (GUIClipCompat.VisibleRect.Contains(Event.current.mousePosition) && data != null)
                                {
                                    DragAndDrop.visualMode = ilvState.rect.Contains(Event.current.mousePosition) ?
                                        DragAndDropVisualMode.Move : DragAndDropVisualMode.None;
                    ...
                    if (isLayouted)
                    {
                        GUILayoutUtilityCompat.EndLayoutGroup();
                        EditorGUILayout.EndScrollView();
                    }
